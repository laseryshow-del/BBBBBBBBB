...
<head>
    ...
    <!-- =================================================================== -->
    <!-- GOOGLE CONSENT MODE V2 DEFAULT - DEBE IR PRIMERO QUE TODO -->
    <!-- =================================================================== -->
    <script>
    ...
    <!-- ELIMINADO: gtag.js hardcodeados de G-J4JTN4JRE0 y G-WXVC7QH9FD    -->
    <!-- (GA4 ya se carga via GTM, tenerlo doble causaba eventos duplicados) -->
    <!-- =================================================================== -->
    <script>
    (function() {
      'use strict';
      ...
      // =====================================================
      // GENERADOR DE EVENT ID UNICO (para deduplicacion)
      // Cada evento DEBE tener su propio event_id unico
      // que coincida entre Browser Pixel y Server CAPI
      // =====================================================
      ...
      // Detectar tipo de pagina automaticamente
      function detectPageCategory() {
        var path = window.location.pathname.toLowerCase();
        if (path === '/' || path === '/index.php' || path === '/inicio') return 'home';
        if (path.indexOf('show') !== -1) return 'show';
        if (path.indexOf('presupuesto') !== -1) return 'presupuesto';
        if (path.indexOf('/dk') !== -1) return 'dk';
      ...
      function detectPageType() {
        var path = window.location.pathname.toLowerCase();
        if (path === '/' || path.indexOf('inicio') !== -1) return 'landing';
        if (path.indexOf('gracias') !== -1 || path.indexOf('thank') !== -1) return 'gracias';
        if (path.indexOf('formulario') !== -1 || path.indexOf('contacto') !== -1) return 'formulario';
        return 'seccion';
      }

      // =====================================================
      // PUSH INICIAL - INCLUYE event_id PARA PAGEVIEW
      // Sin event_id, Meta NO puede deduplicar PageView
      // entre el Pixel del browser y el CAPI del server
      // =====================================================
      var pageViewEventId = window.generateEventId();
      ...
      // Genera event_id unico por cada submit
      // =====================================================
      ...
      // FIX: era 'event_id': event_id (variable inexistente)
      // Corregido a 'event_id': eventId
      // =====================================================
      ...
      // Actualizacion de consentimiento
    ...
    <!-- =================================================================== -->
    <!-- GOOGLE TAG MANAGER - UNICO PUNTO DE CARGA DE TRACKING -->
    <!-- GA4, Meta Pixel, y Server CAPI se manejan SOLO desde GTM -->
    <!-- =================================================================== -->
